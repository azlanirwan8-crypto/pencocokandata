...
| 49 | `api/kodepos-baseline.ts`, `src/utils/kodePosSync.ts` | **N24 — tabel kerja dikuras + dua penjagaan "bersih dan aman".** (a) **Penyebab `kodepos_data` jadi 0 baris, terbukti dari grep seluruh jalur hapus:** satu-satunya kode yang bisa mengurasnya adalah `clearKodePosFromNeon()` → `DELETE /api/kodepos?all=1` → `api/kodepos.ts:637 r.rpc('kp_kosongkan')`, dipanggil HANYA dari handler tombol "Kosongkan Data" (`KodePosManager.tsx:518`, tombol di `:695`) — dan tombol itu **saya perintahkan sendiri** pada laporan N23 ("Menu Data Kode Pos → Kosongkan Data → ya"), jadi ini instruksi operator yang dijalankan, bukan Sync yang menghapus. Audit permukaan Sync (`grep fetchJson('/api/` di `kodePosSync.ts`) menghasilkan 13 URL dan **nol DELETE**; `git status` saat temuan dilaporkan juga masih bersih di `05465dc` (laporan "HEAD 5435f14 + file modified" basi). (b) **Satu jalur hapus massal yang ternyata BISA kepicu dari Sync ditemukan dan dikunci:** `?view=fetch` mengakhiri penarikan dengan `sb.hapus('kodepos_baseline', {versi: neq.<baru>})` — kalau GitHub sedang menolak dan `resolveSource()` jatuh ke mirror komunitas (terukur 81.061 baris vs 83.762 resmi), pembersihan itu menguras patokan resmi. Sekarang hanya jalan bila `source.id === 'kemendagri'`. (c) `salinKoordinatPatokan()` dipanggil TERAKHIR pada jalur isi-baru, bukan sebagai tumpuan: penyalinan window sudah membawa titik sejak INSERT, jadi fungsi penambal itu hanya mengurus baris yang terisi sebelum titik ada, dan kegagalannya (statement besar bisa dipotong batas 8 detik role `anon`) tidak menghentikan pemeriksaan. (d) **Pemulihan dijalankan lewat jalur windowed yang sama, tanpa satu pun DELETE:** crawl 10 tahap × 800 halaman = **7.277/7.277 halaman, 0 gagal, 83.761 titik**, 17,0–34,8 d per panggilan (batas fungsi 60 d); penyalinan 9 tahap = **83.747 baris masuk, 82.867 bertitik**, 3,0–5,3 d per panggilan. **Idempoten terbukti:** seluruh window diulang kedua kalinya → `masuk: 0` di 9/9 tahap, total tetap 83.747. Angka akhir live: `kodeposRecords 83.747` · `patokanTitik 83.761` · `dataTitik 82.867` · `tanpaTitik 880` · `diLuarWilayah 0` · `kodePosCocok 81.904/82.882 (98,8%, sama dengan pengukuran 2026-09-20)`. Selisih `totalBerTitik 82.893` di `?view=stats` vs `dataTitik 82.867` terjelaskan: `kp_stats` memakai `d.latitude IS NOT NULL **OR** g.latitude IS NOT NULL` (join lama ke `kodepos_geo`), `koordinat_cakupan` hanya kolom baris sendiri. Build: `tsc -b --force` 0 error · `tsc -p api` 0 error · lint 60 warning/0 error · build ✓ · 9/9 suite LULUS | kodepos, keamanan data | ✅ selesai |
| 50 | `api/*.ts` (11 file), `src/utils/neonSync.ts`, `src/App.tsx` | **N25 — jalur HAPUS dibetulkan: mode `replace` selama ini selalu gagal senyap (temuan sesi agen lain 2026-09-23, saya konfirmasi statis + perbaiki sampai ke akarnya).** Penyebab: PostgREST **menolak `DELETE` tanpa `WHERE`** (terukur di balasan produksi: `HTTP 500 {"ok":false,"error":"DELETE requires a WHERE clause"}`), sedangkan lapis REST `hapus(tabel, filter)` dipanggil **tanpa filter** pada 9 titik: `api/master.ts:528-529` (cabang `DELETE` / "Kosongkan Data Cabang"), `api/master.ts` cabang `mode:'replace'`, `api/target.ts:545` + `:649` (`final_rows`), `:574` + `:677` (`target_records`), `:678` (`target_meta`), `api/kodepos-baseline.ts:1188` (`kodepos_baseline`). Akibatnya setiap impor "ganti semua" melempar, error tertelan di `try/catch` luar, dan **baris lama tetap berdiri di bawah data baru** — ini juga yang membuat laporan "mode replace master kalah atas data lama" (N17 poin 4) terasa belum sembuh. Perbaikan di akar, bukan per-file: `hapus()` di semua 11 fungsi `api/` kini menolak filter kosong dengan pesan yang menyebut tabelnya, jadi jalur tanpa-WHERE tidak bisa ditambahkan lagi tanpa ketahuan; 9 titik panggilan tadi diberi `WHERE` yang sah (`{id:'gt.0'}` untuk tabel ber-id, `{row_key:'not.is.null'}` untuk `final_rows` karena PK-nya `row_key`, `{key:'not.is.null'}` untuk tabel meta). **Terukur:** `grep -rn "hapus('[a-z_]*')" api/*.ts` → 0 hasil (sebelumnya 9). Bersama itu, dua penjaga anti-timpa Data Cabang dipasang: `loadMasterFromNeon()` membedakan "cloud terbaca kosong" (`rows:[]`) dari "baca gagal" (`null`, bukan array kosong), dan `App.tsx` hanya mendorong salinan browser ke cloud kalau bacaan cloud **berhasil DAN 0 baris** — sebelumnya kegagalan baca bisa dibaca sebagai "cloud kosong" lalu menimpa 1.776 baris `master_records` milik operator. **Sesuai larangan: tidak ada satu pun `DELETE /api/master` yang saya panggil selama perbaikan ini** (verifikasi hanya lewat statis + build), dan `master_records` 1.776 baris tetap utuh terukur sebelum/sesudah. **Selain itu ikut ditutup** karena terukur dari produksi 2026-09-23: `GET ?view=koordinat` (`koordinat_cakupan`) butuh **4,1–5,7 detik** dan **1 dari 4** panggilan nyata dipotong `canceling statement due to statement timeout` — bacaan yang gagal itu dulu berarti `titikAwal = null`, dan `runKodePosLiveSync:638` akan merangkak ulang **7.277 halaman** yang sebenarnya sudah lengkap. `cakupanKoordinat()` kini membaca ulang sekali (+400 ms) sebelum menyatakan tidak tahu. Build: `tsc -p api` 0 error · `tsc -b --force` 0 error · lint 60 warning/0 error · build ✓ · **9/9 suite LULUS**. **Belum terukur:** apakah satu impor `replace` sungguhan kini lolos (hanya bisa dibuktikan oleh operator yang menekan tombol), dan durasi `koordinat_salin()` pada 83.747 baris | P, F1-F6, keamanan data | ✅ selesai (menunggu satu impor `replace` oleh operator) |
| 51 | `api/kodepos-baseline.ts`, `src/utils/kodePosSync.ts`, 8 file `src/**`, `api/master.ts`, `api/target.ts` | **N26 — sumber patokan pindah ke kodepos.co.id + teks "Neon" dibersihkan dari layar** (permintaan pemilik produk 2026-09-23: "kok masih Neon, kan sudah saya bilang tidak pakai Neon lagi", dan "sumber data harus https://kodepos.co.id"). **(a SUMBER)** `POST ?view=situs-baseline {mulai, jumlah, versi}` men-scrape **7.277 halaman kecamatan** situs itu lewat sitemap yang sudah di-cache per instance: payload `self.__next_f` dibaca sebagai JSON terstruktur (`"nama"…"kotaNama"…"provinsiNama"` + `kelurahanList[]` berisi `kodeKemendagri` 13 digit, `kodePos`, `lat`, `lng`, `elevasi`), jadi SATU tarikan menghasilkan patokan 5 level DAN titik koordinatnya sekaligus — bukan dump Kemendagri lagi. `?view=meta.sumber` otomatis berbunyi `kodepos.co.id (scrape 7.277 halaman kecamatan)` karena `base_meta` membaca `max(sumber)` dari tabel itu (bootstrap baris 686). Ditambah `GET ?view=situs-meta` yang **nol tulis** (jumlah URL sitemap + jumlah baris situs yang sudah terpasang) supaya jalur baru bisa dibuktikan live tanpa menyentuh database. Tulis per 500 baris/statement paralel 5 dengan upsert `kode_wilayah` (idempoten), dan **versi lama hanya dibuang kalau versi baru sudah ≥ 60.000 baris** — scrape yang mati di tengah tidak bisa menguras patokan. `pastikanPatokanTerisi()` kini: situs dulu; hanya kalau situs gagal/tipis dump Kemendagri dipakai, dan laporan menuliskannya sebagai `SUMBER CADANGAN …` beserta angka barisnya. **Tumpang-tindih terukur SEBELUM ganti sumber (read-only, provinsi Bali: 57 halaman situs vs `kodepos_data` produksi): 716 baris vs 716 baris; kunci-5 sama 658 (91,9%); 58 baris hanya ada di situs; 58 hanya ada di produksi; 0 baris "3 kolom sama tapi 5 kolom beda"; 0 kembar di kedua sisi; 716/716 baris situs punya titik valid.** Karena pengisian tabel kerja bersifat menambah (`import-missing`), satu Sync penuh akan menaikkan baris ±8% se-Indonesia dan meninggalkan baris lama yang tidak lagi disebut situs; rebuild bersih tetap lewat "Kosongkan Data" yang ditekan operator sendiri — jalur Sync tidak menghapus tabel kerja. **(b NEON)** 55 teks/komentar `Neon` diganti (4 suntingan langsung + 34 + 8 + 9 dari tiga sapuan): **6 string yang DIRENDER** — `KodePosSyncModal.tsx:263`, `:271`, `:317`, `:231`, `IndonesiaBranchMap.tsx:366`, `onlineGeoCoder.ts:136` — + 25 `console.warn` + sisanya komentar menyesatkan dan 6 nama berkas bawaan `Master/Target_Neon*.xlsx` → `..._Supabase.xlsx`, termasuk yang dikirim server di `api/master.ts:420/473` dan `api/target.ts:451/569` sehingga UI menampilkan nama berkas berbau Neon. Verifikasi: grep string literal/teks JSX memuat `Neon` di `src/**` = **0**; di `api/*.ts` = **0**. Identifier internal (`neonSync.ts`, `saveMasterToNeon`, `isNeonConnected`, `NeonDatabaseModal`) TIDAK direname — di luar permintaan dan berisiko memutus jalur tulis di tengah pekerjaan sesi lain. **(c `terakhir` — TIDAK ada bug)** `terakhir` = `max(diambil_pada)` dari `kodepos_koordinat` (`server/supabase-bootstrap.sql:738`; kolomnya `timestamptz`, bootstrap baris 192; dibaca `api/kodepos-baseline.ts` → `terakhir: s.terakhir`). Selisih "±8 jam" yang dilaporkan berasal dari JAM MESIN KERJA: `date -u` laptop = 13:32:57 sedang `serverTime` Postgres = 05:32:47 → mesin +8j00m. Crawl nyata menulis titik terakhir 04:09Z dan "sekarang" saat itu 04:17Z, jadi angkanya memang segar. Build: `tsc -p api` 0 error · `tsc -b --force` 0 error · lint 60 warning/0 error · build ✓ · **9/9 suite LULUS**. **Belum terukur:** satu tekan Sync penuh dengan sumber baru (±10 panggilan crawl + delta nasional), dan berapa besar selisih tipe "58 baris Bali" itu di 38 provinsi | kodepos, sumber data, UI | ✅ selesai (menunggu satu tekan Sync oleh operator) |
