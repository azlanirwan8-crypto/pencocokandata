...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      `;
      return res.status(200).json({ ok: true, configured: true, message: 'Data master RoleMapping berhasil disimpan ke Neon Postgres.' });
    }

    if (req.method === 'DELETE') {
      await sql`DELETE FROM app_store WHERE key = 'rolemapping_data';`;
      return res.status(200).json({ ok: true, configured: true, message: 'Data master RoleMapping berhasil dibersihkan dari Neon Postgres.' });
    }
