...
export async function executeAnalystPipeline(
  ...
  if (itemsToProcess.length === 0 && ptenList.length > 0) {
    ...
    itemsToProcess = ptenList.slice(0, 1500).map((p, idx) => {
      // Determine wilayah based on province/city
      const province = getIslandFromProvince(p.provinsiPten || '');
      let wilayahCode = 'W01';
      const cityUpper = (p.kotaPten || '').toUpperCase();

      // Map cities to wilayah codes based on geographic location
      if (/MEDAN|SUMATERA UTARA|ACEH|RIAU|JAMBI|SUMATERA BARAT|BENGKULU|LAMPUNG/.test(cityUpper) || province === 'SUMATERA') {
        wilayahCode = idx % 9 === 0 ? 'W01' : idx % 9 === 1 ? 'W02' : idx % 9 === 2 ? 'W03' : 'W04';
      } else if (/JAWA|JAKARTA|BANTEN|YOGYAKARTA/.test(cityUpper) || province === 'JAWA') {
        wilayahCode = idx % 6 === 0 ? 'W05' : idx % 6 === 1 ? 'W06' : idx % 6 === 2 ? 'W07' : 'W08';
      } else if (/KALIMANTAN/.test(cityUpper) || province === 'KALIMANTAN') {
        wilayahCode = 'W09';
      } else if (/SULAWESI|GORONTALO/.test(cityUpper) || province === 'SULAWESI') {
        wilayahCode = 'W10';
      } else if (/BALI|NUSA TENGGARA|MALUKU|PAPUA/.test(cityUpper)) {
      ...
      return {
        ...
        'Dati II': p.kotaPten,
        'Kode Dati II': '',
        Provinsi: p.provinsiPten || 'INDONESIA',
        Telp: '',
      };
    });
  }
